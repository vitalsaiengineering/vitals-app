...
export default function ClientInceptionView({ globalSearch }: ClientInceptionViewProps) {
  const [inceptionData, setInceptionData] = useState<ClientInceptionData | null>(null);
  ...
  useEffect(() => {
    ...
    if (selectedSegmentFilter !== 'All Segments') params.segmentFilter = selectedSegmentFilter;
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-lg font-medium">Clients by Inception Date by Segmentation</h3>
              </div>
              <div className="mt-4">
                <p className="text-3xl font-bold">{inceptionData?.kpi.ytdNewClients || 0}</p>
                <p className="text-sm text-muted-foreground">YTD New Clients</p>
                <div className="flex items-center mt-1">
                  <TrendingUp className="h-4 w-4 text-green-600 mr-1" />
                  <span className="text-sm text-green-600 font-medium">
                    +{inceptionData?.kpi.percentageChangeVsPreviousYear || 0}% vs previous year
                  </span>
                </div>
              </div>
            </div>
        ...
        <CardContent>
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={inceptionData?.chartData || []}>
                    ...
                    <Bar dataKey="Platinum" stackId="a" fill="hsl(222, 47%, 44%)" />
                    <Bar dataKey="Gold" stackId="a" fill="hsl(216, 65%, 58%)" />
                    <Bar dataKey="Silver" stackId="a" fill="hsl(210, 55%, 78%)" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
                  ...
                  <div key={index} className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      <div 
                        ...
                        style={{ 
                          backgroundColor: item.segment === 'Platinum' ? 'hsl(222, 47%, 44%)' :
                                          item.segment === 'Gold' ? 'hsl(216, 65%, 58%)' :
                                          item.segment === 'Silver' ? 'hsl(210, 55%, 78%)' :
                                          '#6b7280'
                        }}
                      />
          ...
          <div className="flex space-x-2 mt-6">
            {segmentButtons.map((segment) => (
              <Button
                key={segment}
                variant={selectedSegmentFilter === segment ? "default" : "outline"}
                size="sm"
        ...
        <CardHeader>
          <CardTitle>
            ...
            <span className="text-sm font-normal text-muted-foreground ml-2">
              {inceptionData ? `${inceptionData.totalTableRecords} records` : 'Loading...'} • {selectedSegmentFilter} - {selectedYear}
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading && <div className="p-4 text-center text-muted-foreground">Loading client data...</div>}
          {!isLoading && inceptionData && inceptionData.tableClients.length === 0 && (
                  ...
                  {inceptionData.tableClients.map((client) => {
                    ...
                    return (
                      <TableRow key={client.id}>
                        ...
                        <TableCell>
                          {new Date(client.inceptionDate).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'short',
                            day: 'numeric'
                          })}
