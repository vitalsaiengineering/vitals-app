...
const getMockAgeDemographicsReportData = async (
  ...
  const updatedByAgeBracket = byAgeBracketData.map((entry) => {
    ...
    if (count > 0 && numSegments > 0) {
      // count is the actual number of clients in this age bracket
  ...
  const averageClientAge = parseFloat(
    (
      clients.reduce((sum, client) => sum + client.age, 0) / totalClients || 0
    ).toFixed(1)
  );
  ...
  return {
    ...
    clientDetails: clients.map((client: any, index: number) => {
      ...
      return {
        ...
        name:
          client.name ||
          `${client.firstName || ""} ${client.lastName || ""}`.trim(), // Use actual client name or construct it
...
async function getMockClientDistributionReportData(
  organizationId: number
...
async function getMockBookDevelopmentReportData(
  organizationId: number
...
async function getClientSegmentationDashboardHandler(
  req: Request,
  res: Response
) {
  const user = req.user as any;
  ...
  try {
    ...
    const segmentTotals = {
      Platinum:
        allClientsMock.Platinum.reduce(
    ...
    const mockData: ClientSegmentationDashboardData = {
      ...
      advisorOptions,
      currentAdvisorOrFirmView:
        advisorFilter === "firm_overview"
          ? "Firm Overview"
          : advisorOptions.find((opt) => opt.id === advisorFilter)?.name ||
            "Firm Overview",
    };
...
async function getClientInceptionHandler(req: Request, res: Response) {
  try {
    ...
    const mockChartLegend: InceptionChartLegendItem[] =
      legendDataForSelectedYear
        ? [
    ...
    let filteredTableClients = allMockTableClients.filter(
...
// --- START: Interfaces for Referral Analytics ---
interface ReferralClient {
  id: string;
  clientName: string;
  segment: "Platinum" | "Gold" | "Silver";
  referredBy: string;
  primaryAdvisor?: string; // New field for primary advisor
  aum: number;
  referralDate: string;
}
...
// --- END: Interfaces for Referral Analytics ---

// --- START: Handler for Referral Analytics ---
async function getReferralAnalyticsHandler(req: Request, res: Response) {
  try {
    ...
    if (referrer && typeof referrer === "string" && referrer !== "all") {
      ...
      const referrerName = referrerMap[referrer];
      if (referrerName) {
        filteredReferrals = filteredReferrals.filter(
...
// --- END: Handler for Referral Analytics ---

// --- START: Interfaces for Client Referral Rate ---
interface ReferralRateKPI {
  currentRate: number;
  rateChange: number;
...
interface ReferralRateDataPoint {
  month: string;
  shortMonth: string;
  referralRate: number;
...
// --- END: Interfaces for Client Referral Rate ---

// --- START: Mock Data and Handler for Client Referral Rate ---
async function getMockClientReferralRateData(
  organizationId: number
): Promise<ClientReferralRateData> {
  // Mock historical data for referral rate trends
  const mockChartData: ReferralRateDataPoint[] = [
    { month: 'Jan 2024', shortMonth: 'Jan', referralRate: 27, referredClients: 8, totalNewClients: 30 },
    { month: 'Feb 2024', shortMonth: 'Feb', referralRate: 42, referredClients: 12, totalNewClients: 29 },
    { month: 'Mar 2024', shortMonth: 'Mar', referralRate: 55, referredClients: 16, totalNewClients: 29 },
    { month: 'Apr 2024', shortMonth: 'Apr', referralRate: 42, referredClients: 11, totalNewClients: 26 },
    { month: 'May 2024', shortMonth: 'May', referralRate: 57, referredClients: 17, totalNewClients: 30 },
    { month: 'Jun 2024', shortMonth: 'Jun', referralRate: 59, referredClients: 19, totalNewClients: 32 },
    { month: 'Jul 2024', shortMonth: 'Jul', referralRate: 64, referredClients: 18, totalNewClients: 28 },
    { month: 'Aug 2024', shortMonth: 'Aug', referralRate: 62.1, referredClients: 18, totalNewClients: 29 },
    { month: 'Sep 2024', shortMonth: 'Sep', referralRate: 68, referredClients: 20, totalNewClients: 29 },
    { month: 'Oct 2024', shortMonth: 'Oct', referralRate: 72, referredClients: 23, totalNewClients: 32 },
    { month: 'Nov 2024', shortMonth: 'Nov', referralRate: 76, referredClients: 25, totalNewClients: 33 },
    { month: 'Dec 2024', shortMonth: 'Dec', referralRate: 83.6, referredClients: 27, totalNewClients: 32 },
  ];
  ...
  // In a real application, you could modify this data based on organizationId
  // to show different metrics per organization

  return {
    kpi: {
      currentRate,
      rateChange: Math.round(rateChange * 10) / 10, // Round to 1 decimal
      newClientsThisMonth,
...
async function getClientReferralRateHandler(req: Request, res: Response) {
  const user = req.user as any;
  const organizationId = user?.organizationId;
  const advisorIdQuery = req.query.advisorId as string | undefined;
  const advisorId = advisorIdQuery ? parseInt(advisorIdQuery, 10) : undefined;
...
interface AdvisoryFirmDashboardData {
  totalActivities: number;
...
// --- END: Interfaces for Advisory Firm Dashboard ---

// --- START: Mock Data and Handler for Advisory Firm Dashboard ---
async function getMockAdvisoryFirmDashboardData(
  organizationId: number
  ...
  const mockData: AdvisoryFirmDashboardData = {
    ...
    messagesSent: 523,
    topPerformers: [
      { id: 'sj', initials: 'SJ', name: 'Sarah Johnson', totalActivities: 189, meetings: 24, emails: 87, tasks: 45 },
      { id: 'mc', initials: 'MC', name: 'Michael Chen', totalActivities: 167, meetings: 19, emails: 92, tasks: 38, isHighlighted: true },
      { id: 'er', initials: 'ER', name: 'Emily Rodriguez', totalActivities: 154, meetings: 22, emails: 76, tasks: 41 },
      { id: 'dk', initials: 'DK', name: 'David Kim', totalActivities: 142, meetings: 18, emails: 69, tasks: 39 },
      { id: 'lt', initials: 'LT', name: 'Lisa Thompson', totalActivities: 138, meetings: 16, emails: 71, tasks: 35 }
    ],
    needsAttention: [
      { id: 'rw', initials: 'RW', name: 'Robert Wilson', totalActivities: 67, meetings: 8, emails: 34, tasks: 18 },
      { id: 'jd', initials: 'JD', name: 'Jennifer Davis', totalActivities: 54, meetings: 6, emails: 28, tasks: 15, isHighlighted: true },
      { id: 'ma', initials: 'MA', name: 'Mark Anderson', totalActivities: 42, meetings: 5, emails: 22, tasks: 12 }
    ],
    monthlyData: [
    ...
    staffDetails: [
      {
        ...
        name: 'Jennifer Davis',
        activities: [
          { activityType: 'Client Meetings', mtd: 1, qtd: 3, ytd: 10, ttm: 11, trend: 'up' },
      ...
      },
      {
        ...
        name: 'Michael Chen',
        activities: [
          { activityType: 'Client Meetings', mtd: 8, qtd: 25, ytd: 102, ttm: 110, trend: 'up' },
...
async function getAdvisoryFirmDashboardHandler(req: Request, res: Response) {
  const user = req.user as any;
  const organizationId = user?.organizationId;
  const advisorIdQuery = req.query.advisorId as string | undefined;
  const advisorId = advisorIdQuery ? parseInt(advisorIdQuery, 10) : undefined;
...
// --- END: Mock Data and Handler for Advisory Firm Dashboard ---
